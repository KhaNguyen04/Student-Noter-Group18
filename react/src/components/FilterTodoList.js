...
const TodoList = (props) => {
  ...
  //   props.setTodos(newTodos);
  // };
  const adjustTask = (description) => {
    const data = {
      "id": currentAdjustInfo['id'],
      "task": currentAdjustInfo['task'],
      "description": description,
      "category": currentAdjustInfo['category'],
      "time": currentAdjustInfo['time'],
      "date": currentAdjustInfo['date'],
      "finished": currentAdjustInfo['finished']
    };
    const newTodos = [...props.todos];
    const index = props.todos.findIndex((contact) => contact.id === currentIDToEdit);
    newTodos.splice(index, 1, data);
    props.setTodos(newTodos);
  };
  
  const handleAdjustClick = (id, description) => {
    console.log(id);
    ...
    const data = {
      "id": index,
      "task": props.todos[index]['task'],
      "description": description,
      "category": props.todos[index]['category'],
      "time": props.todos[index]['time'],
      "date": props.todos[index]['date'],
  ...
  return (
    <div className="main">
      ...
      <Popup show={showAdjustPopup} onHide={() => {setShowAdjustPopup(false);}} components={
        <AdjustForm adjustTask={(taskInfo) => adjustTask(taskInfo)} adjustInfo={currentAdjustDescription2} setCurrentAdjustInfo={setCurrentAdjustInfo} currentAdjustDescription={currentAdjustDescription} setCurrentAdjustDescription={setCurrentAdjustDescription} categories={props.categories} onHide={() => {setShowAdjustPopup(false);}}/>
      }/>
